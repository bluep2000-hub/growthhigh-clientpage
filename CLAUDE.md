...
**고친 것은 「저장」을 눌러야 나간다.** 그전까지는 화면에만 쌓이고, 고친 줄에는
왼쪽에 표시가 붙는다. 잘못 쳤으면 「되돌리기」로 한꺼번에 물린다. 저장하지 않고
화면을 떠나려 하면 붙잡는다. 보태기와 지우기만 아직 즉시 나간다.
...
한 번에 저장할 수 있는 줄은 스무 줄이다. 넘으면 위에서부터 스무 줄이 나가고
남은 수를 알려 준다 — 저장 한 번이 재빌드 한 번이라 몰래 나눠 보내지 않는다.
