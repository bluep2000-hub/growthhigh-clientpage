...
SOURCE_SLUG = "dongseon-src"       # 복제할 원본 클라이언트 (임시 클라이언트페이지)
SAMPLE_SLUG = "it"                 # 산출물 c/{이것}.json · assets/notice/{이것}-*
...
NEW_NAME = "로컬링크"
NEW_EN = "LocalLink"
NEW_EN_CORP = "LocalLink Co.,Ltd."
NEW_DOMAIN = "locallink.co.kr"

# 원본에서 지울 표기. 긴 것부터 쓴다 — 앞의 규칙이 뒤를 먹지 않게.
SRC_NAMES = ["주식회사 동선컴퍼니", "주식회사동선컴퍼니",
             "(주)동선컴퍼니", "동선컴퍼니"]
SRC_BRANDS = ["동네기프트", "동네 기프트"]   # 서비스 표기. 도메인은 SRC_DOMAINS 가 맡는다
SRC_DOMAINS = ["dongne.gift"]
SRC_EN = [r"dongne", r"local\s*link"]      # 영문 상호. 정규식이라 대소문자·띄어쓰기를 가린다

# 지우지 않고 가명으로 바꿀 고유명.
SRC_ALIASES = []

# 공용 메일에 쓰는 개인 계정. 로컬파트에 사명·실명이 들어가면 도메인 치환만으로는 남는다.
SRC_ACCOUNTS = {"ceo@dongne.gift": "ceo@locallink.co.kr"}
...
PROGRAM_WORDS = [
    "초기창업패키지", "창업중심대학", "창업성장기술개발", "디딤돌",
    "청년창업사관학교", "관광벤처지원사업", "소상공인", "신용보증기금",
    "정책자금", "R&D", "PSST", "사업계획서", "기업부설연구소",
    "이노비즈", "메인비즈", "벤처인증", "벤처기업확인서",
    "중소기업확인서", "창업기업확인서", "회사소개서", "조직도", "주주명부",
]
...
# 데모 로고. None 이면 화면이 기업명 앞 두 글자를 대신 그린다.
SAMPLE_LOGO = None
SAMPLE_LINKS: list[tuple[str, list[tuple[str, str]]]] = []

# 거래처·제3자 상호와 도메인. 데모에 필요 없다.
THIRD_PARTY = [
    (r"\b(nate\.com)\b", "example.com"),
    # 노션 사이트 주소는 가이드북 한 장만 남기고 전부 지운다.
    (rf"https?://[\w.-]*\.notion\.site/(?!{KEEP_NOTION_PAGE})\S*",
     "https://example.com/"),
]

# 만들고 나서 전문을 훑는다. 하나라도 남으면 배포하지 않는다.
FORBIDDEN = ["동선컴퍼니", "dongseon", "Dongseon", "DONGSEON",
             "dongne.gift", "dongne", "김성민", "미유로", "정왕동",
             "010-4274", "4274-2111"]
