...
| `GET` | `/notice/item?slug=&blockId=` | 그 항목을 고칠 때 입력칸에 넣을 글 |
| `PUT` | `/notice/item` | 그 항목을 고친다 |

보태기(`POST`)와 지우기(`DELETE`)는 티켓 #13 에서 붙는다.
...
|---|---|
| `401` | 비밀번호가 없거나 틀리다 |
| `404` | 모르는 슬러그 · 그 기업 공지에 없는 blockId · 공지 항목이 아닌 블록 |
| `409` | 잠긴 항목 — 링크·이모지·밑줄·글자색이 들어 있다 |
