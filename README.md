...
## 소통 내역 — PM이 공개한 기록만 고객에게 보인다

메일은 먼저 원본 아카이브에 보관하고, 커뮤니케이션보드에는 `고객 공개=true`로
넣는다. 숨겨야 할 메일만 담당 PM이 `고객 공개` 체크를 해제한다.

```
다우오피스 Inbox.고객사.{기업명}
또는 Inbox.그로스하이의 제목 [기업명]
        │  ← 빌더가 헤더 + 본문을 읽는다 (읽기 전용)
        ▼
   노션 커뮤니케이션보드    고객 공개 = 체크
        │
        │  ← 숨길 메일만 담당 PM이 「고객 공개」를 해제한다
        ▼
   고객 보안 서버 → 화면      승인된 기록만
```
...
**폴더 매칭** — 기업 DB의 기업명과 전용 폴더명이 정확히 일치하면 그 폴더를 쓴다.
전용 폴더가 없으면 공용 `Inbox.그로스하이`에서 제목의 `[기업명]` 말머리가 정확히
일치하는 메일만 수집한다. 발신 도메인이나 이름 일부로 추측하지 않는다.
